...
class CurlClient(object):
    ...
    def postXmlSSL(self, xml, url, second=30, cert=True, post=True):
        ...
        if cert:
            ...
            self.curl.setopt(pycurl.SSLKEY, settings.WX_SSLKEY_PATH)
            self.curl.setopt(pycurl.SSLCERTTYPE, "PEM")
            self.curl.setopt(pycurl.SSLCERT, settings.WX_SSLCERT_PATH)
        #post提交方式
        if post:
...
class HttpClient(Singleton):
    @classmethod
    def configure(cls):
        if pycurl is not None and settings.WX_HTTP_CLIENT != "URLLIB":
            return CurlClient
        else:
            return UrllibClient
...
class Common_util_pub(object):
    ...
    def createNoncestr(self, length = 32):
        """产生随机字符串，不长于32位"""
        # return "ufgcp4gxkewtkt3h6jalik3j2t70e1u3"
    ...
    def getSign(self, obj):
        ...
        #logging.debug("getSign step-1: %s" % String)

        #签名步骤二：在string后加入key
        String = "{0}&key={1}".format(String, settings.WX_KEY)

        #logging.debug("getSign step-2: %s" % String)

        #签名步骤三：MD5加密
        String = hashlib.md5(String).hexdigest()

        #logging.debug("getSign step-3: %s" % String)

        #签名步骤四：所有字符转为大写
        result_ = String.upper()

        #logging.debug("getSign step-4: %s" % result_)
...
class JsApi_pub(Common_util_pub):
    ...
    def __init__(self, timeout=settings.WX_CURL_TIMEOUT):
        self.curl_timeout = timeout

    def createOauthUrlForCode(self, redirectUrl):
        """生成可以获得code的url"""
        urlObj = {}
        urlObj["appid"] = settings.WX_APPID
    ...
    def createOauthUrlForOpenid(self):
        ...
        urlObj = {}
        urlObj["appid"] = settings.WX_APPID
        urlObj["secret"] = settings.WX_APPSECRET
    ...
    def  getParameters(self):
        """设置jsapi的参数"""
        jsApiObj = {}
        jsApiObj["appId"] = settings.WX_APPID
...
class Wxpay_client_pub(Common_util_pub):
    ...
    def createXml(self):
        ...
        self.parameters["appid"] = settings.WX_APPID   #公众账号ID
        self.parameters["mch_id"] = settings.WX_MCHID   #商户号
    ...
    def getResult(self):
        ...
        #logging.debug("getResult: %r" % self.result)
...
class RefundQuery_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        """生成接口参数xml"""
        if all(self.parameters.get(key) is None for key in ("out_refund_no", "out_trade_no", "transaction_id", "refund_id")):
            raise ValueError("missing parameter")
        self.parameters["appid"] = settings.WX_APPID  #公众账号ID
        self.parameters["mch_id"] = settings.WX_MCHID  #商户号
        self.parameters["nonce_str"] = self.createNoncestr()  #随机字符串
...
class NativeCall_pub(Wxpay_server_pub):
    ...
    def createXml(self):
        ...
        if self.returnParameters["return_code"] == self.SUCCESS:
            self.returnParameters["appid"] = settings.WX_APPID #公众账号ID
            self.returnParameters["mch_id"] = settings.WX_MCHID #商户号
